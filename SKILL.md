...
>
> 推荐先看使用说明（浏览器打开）：`README.html`（和本文件同目录）
...
**触发：** 用户输入 `/sync-profile merge`

**适用场景：** 家里和公司两个环境的种子包各自有增删改，需要合并。

**步骤：**

1. 确认本地种子包路径（默认自身）和外部种子包路径（如 U 盘 `E:\sync-profile\`）
2. 运行 `scripts/compute-hash.ps1` 分别计算两个种子包各文件的 MD5
3. 对比两边的 `seed.yaml` 中的 `files` 校验码，找出差异：
   - **hash 相同** → 跳过，无需处理
   - **值为 `SKIP_SELF_HASH`** → 跳过（seed.yaml 自身哈希每次编辑都会变，用占位符标记）
   - **单边新增**（文件只在 A 有，B 没有）→ 列出，问是否复制到另一边
   - **两边都改了**（hash 都与 seed.yaml 不同）→ 逐文件展示 diff，问用户保留哪边
4. 对每个有差异的文件：
   - 展示文件路径 + 两边各自的最后修改时间（如有）或 hash 值
   - 用 `diff` 展示内容差异（行级别的增删）
   - 给用户三个选项：保留 A、保留 B、手动编辑合并
5. 如果选了手动编辑合并，AI 打开编辑器或直接展示合并后的内容请用户确认
6. 所有冲突解决后，重新运行 `scripts/compute-hash.ps1` 更新 `seed.yaml`
7. 更新 seed.yaml changelog
8. 告知用户本次 merge 的变更摘要

> **原则：** merge 操作只负责比对和展示差异，所有冲突解决最终由用户决策。AI 可以做三向合并建议，但不替用户做决定。
